...
unique_first_three_words = (
    df["question"].apply(lambda x: " ".join(x.split()[:3])).unique()
)
...
skills_to_remove = [
    ...
    "Is the sentence simple compound, comples, or compound-complex",
    "Use guide words",
]
...
df_reformatted["correct_answer"] = df_reformatted.apply(
...
df_reformatted["choices"] = df_reformatted["choices"].apply(
...
df_reformatted = df_reformatted.rename(
    columns={
...
df_reformatted = df_reformatted[
    ["instruction", "input", "output", "correct_answer", "answer"]
]
...
print(
    "There are ",
    len(output_lengths[output_lengths > 40]),
    "examples with output length > 40",
)
...
for input in df_reformatted["input"].unique():
    print(
        "There are",
        len(df_reformatted[df_reformatted["input"] == input]),
        "examples with input:",
        input[:60],
    )
    print("\n")
...
df_reformatted["contains_description"] = df_reformatted["instruction"].apply(
    check_description
)
...
df_reformatted.drop(
    ["correct_answer", "answer", "contains_description"], axis=1
).to_json("scienceqa_formatted.json", orient="records", indent=1)
